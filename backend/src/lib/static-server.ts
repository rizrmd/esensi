 ...
 */
function getMimeType(filePath: string, customMimeTypes?: Record<string, string>): string {
  const ext = filePath.substring(filePath.lastIndexOf(".")).toLowerCase();
  return (customMimeTypes && customMimeTypes[ext]) || defaultMimeTypes[ext] || "application/octet-stream";
}
    ...
    try {
      ...
      try {
        ...
        if (stats.isDirectory()) {
          ...
          return new Response("Directory listing not implemented", { status: 501 });
        }
