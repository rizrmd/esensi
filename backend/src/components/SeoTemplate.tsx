...
export const SeoTemplate = (arg: { data: any }) => {
    ...
    const meta_img = data.image ? data.image.replace("_file/", "https://esensi.online/_img/") : "https://esensi.online/assets/img/logo.png";
    ...
    if( data.page && data.page !== "undefined" && data.page !== "null" && data.page > 1 ) {
    ...
    let the_title = data.title ? data.title : data.meta_title ? data.meta_title : data.headings ? data.headings : data.h1;
    if (the_title.length > title_length) {
        the_title = the_title.substring(0, title_length) + meta_page;
    }

    let the_desc = data.meta_description ? data.meta_description : data.description;
    the_desc = the_desc.replace(/(<([^>]+)>)/gi, "").replace(`'`,``).replace(`"`,``);
    ...
    if (data?.keywords && data?.keywords !== "undefined" && data?.keywords !== "null") {
    ...
    return (
        <Fragment>
            <html lang="id">
                ...
                <body>
                    <h1>{data?.h1 ? data.h1 : data.headings ? data.headings : data.meta_title}</h1>
                    <h2>{data?.h2 ? data.h2 : data.headings ? data.headings : data.meta_title}</h2>
                    <h3>{data?.h3 ? data.h3 : data.headings ? data.headings : data.meta_title}</h3>
