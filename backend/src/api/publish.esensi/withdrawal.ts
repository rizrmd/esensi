  ...
  async handler(arg: { 
    ...
    try {
      ...
      if (!authUser?.id_publisher) {
        return { success: false, message: "Hanya penerbit yang dapat mengakses API ini" };
      }

      const publisherId = authUser.id_publisher;

      // Check if the amount is valid
      if (!arg.withdrawal.amount || isNaN(arg.withdrawal.amount) || arg.withdrawal.amount <= 0) {
      ...
      if (arg.withdrawal.amount > Number(balance)) {
        return { success: false, message: "Jumlah penarikan melebihi saldo tersedia" };
      }
      ...
      return { success: false, message: "Gagal memproses permintaan penarikan" };
    }
  },
});