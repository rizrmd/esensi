...
export default defineAPI({
  ...
  async handler(arg: CheckOrderStatusRequest): Promise<ApiResponse<OrderStatusResponse>> {
    try {
      // Validasi user login
      if (!arg.user?.id) {
        return {
          success: false,
          message: "User harus login untuk mengecek status pesanan",
        };
      }
      ...
      if (salesRecord.status === "pending" || salesRecord.status === "challenge") {
        try {
