...
export default defineAPI({
  ...
  async handler(arg: CreatePaymentRequest): Promise<ApiResponse<CreatePaymentResponse["data"]>> {
    try {
      // Validasi user login
      if (!arg.user?.id) {
        return {
          success: false,
          message: "User harus login untuk melakukan pembayaran",
        };
      }
      ...
      if (cartProductIds.length > 0) {
        ...
        for (const product of products) {
          const cartItem = arg.cart_items.find(item => item.id === product.id);
          if (cartItem) {
      ...
      const customerDetails = {
        first_name: arg.customer_info?.first_name || authUser?.name?.split(" ")[0] || customer.name.split(" ")[0] || "Customer",
        last_name: arg.customer_info?.last_name || authUser?.name?.split(" ").slice(1).join(" ") || customer.name.split(" ").slice(1).join(" ") || "",
        email: arg.customer_info?.email || authUser?.email || customer.email || "",
        phone: arg.customer_info?.phone ? MidtransService.formatPhoneNumber(arg.customer_info.phone) : customer.whatsapp || "",
      };
