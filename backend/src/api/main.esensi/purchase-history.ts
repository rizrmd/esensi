...
export default defineAPI({
  ...
  async handler(arg: PurchaseHistoryRequest): Promise<ApiResponse<PurchaseHistoryResponse>> {
    try {
      // Validasi user login
      if (!arg.user?.id) {
        return {
          success: false,
          message: "User harus login untuk melihat riwayat pembelian",
        };
      }
      ...
      // Transform data
      const purchaseItems: PurchaseHistoryItem[] = purchases.map(purchase => ({
        id: purchase.id,
        ...
        updated_at: purchase.updated_at?.toISOString() || purchase.created_at.toISOString(),
        ...
        payment_type: (purchase.midtrans_success as any)?.payment_type || 
