...
export const betterAuth = {
  ...
  getSession: async (): Promise<AuthClientGetSessionAPIResponse> => {
    ...
    if (data?.user) {
      ...
      const _user = await api.user_get({ id: user.id });
      user.affiliate = _user.data?.affiliate;
      user.author = _user.data?.author;
      user.customer = _user.data?.customer;
      user.publisher = _user.data?.publisher;
      user.internal = _user.data?.internal;
      data.user = user;
    }
    return { data, error };
  },
