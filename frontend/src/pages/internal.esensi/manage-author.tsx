...
import { author, Item } from "@/components/ext/author/item-manage";
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="mx-8 py-8">
                <div className="flex justify-between items-start mb-8 gap-4">
                  <h1 className="text-2xl font-bold">Daftar Penulis</h1>
                  <DataPagination
                    total={local.total}
                    page={local.page}
                    limit={local.limit}
                ...
                {local.loading ? (
                  <div>Mengambil data penulis...</div>
                  ...
                  <>
                    {local.layout === ItemLayoutEnum.GRID && (
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
                        {local.authors.map((item: Author) => (
                          <div
                            key={item.id}
                            className="cursor-pointer"
                            onClick={() =>
                              navigate(`/author-detail?id=${item.id}`)
                            }
                          >
                            <Card className="flex flex-col h-full shadow-md border border-gray-200 hover:shadow-lg transition-shadow">
                              ...
                              <CardContent className="pb-4">
                                <Item type={local.layout} item={author(item)} />
                              </CardContent>
                            </Card>
                          </div>
                        ))}
                    ...
                    {local.layout === ItemLayoutEnum.COMPACT && (
                      <div className="border rounded-md overflow-hidden">
                        <table className="w-full">
                          ...
                          <tbody>
                            {local.authors.map((item: Author, index) => (
                              <tr
                                key={item.id}
