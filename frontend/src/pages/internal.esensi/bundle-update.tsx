...
import { Currency } from "backend/lib/types";
...
export default () => {
  const local = useLocal(
    ...
    async () => {
      ...
      await Promise.all([loadBundleData(), loadAuthors()]);
      local.loading = false;
      local.render();
    }
  );
  ...
  const loadBundleData = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        local.selectedAuthor = response.data.id_author || "";
        ...
        // Load products for the selected author
        if (local.selectedAuthor) await loadProducts(local.selectedAuthor);
  ...
  const loadAuthors = async () => {
    try {
      const response = await api.author_list({ limit: 100 });
  ...
  const onAuthorChange = async (authorId: string) => {
    ...
    local.products = [];
    if (authorId) await loadProducts(authorId);
    local.render();
  };
