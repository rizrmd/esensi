...
export default () => {
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="relative">
              ...
              <FormDescription><Link href="/forgot" className="text-xs text-[#3B2C93] font-bold">Lupa kata sandi?</Link></FormDescription>
              <FormMessage />
            </FormItem>
          )}
        />
  ...
  const renderRegisterHeader = (
    <div className="flex w-full flex-col items-center gap-6 text-[#3B2C93]">
      <div
        className="w-1/4 [&>svg]:h-auto [&>svg]:w-full"
        dangerouslySetInnerHTML={{ __html: logo }}
      ></div>
        <h2 className="whitespace-pre-line text-3xl text-center font-semibold">Selamat Datang 
          di Esensi online</h2>
    </div>
  );
