...
function PageHeader({ title="Esensi Online" as string, back=true as boolean, logo=true as boolean, search=true as boolean }) {
  ...
  const the_title = <div className="flex flex-1 justify-start items-center pl-1 pr-1 font-bold">{pageTitle}</div>
  const the_search = showSearch ? <Button onClick={(e) => {
    e.preventDefault();
    setIsSearching(!isSearching);
  }} variant="ghost" className="flex h-full w-auto justify-center items-center gap-1 cursor-pointer pl-1 pr-1 rounded-none">Cari buku <Search size={20} strokeWidth={1.5} /></Button> : "";
...
function BookCard( {data} ){
  ...
  if (data.strike_price !== null && data.strike_price !== "" && data.strike_price > data.real_price) {
    ...
    strikePrice = <div className="w-auto line-through text-[11px] text-[#a9a9a9]">{formatMoney(data.strike_price, data.currency)}</div>
  }
...
function CategoryBooks({category = "", title = "", subtitle = "", max = 10, link = true }) {
  ...
  const the_subtitle = theSubtitle !== "" ? <div className="flex flex-1 justify-start text-[11px]">{theSubtitle}</div> : "";
