...
export const CfgEditor = ({ book, canEdit = true, onSave }: CfgEditorProps) => {
  const local = useLocal(
    ...
    async () => {
      ...
      const cfg = (book.cfg as Record<string, any>) || {};
  ...
  const saveCfg = async () => {
    ...
    try {
      ...
      const result = await api.book_update({
        id: book.id,
        data: { cfg },
      });
      ...
      if (result.success && result.data) {
        ...
        local.error = result.message || "Gagal menyimpan konfigurasi";
      }
    } catch (error) {
    ...
    <Card className="shadow-md border border-gray-200 mt-6">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg">Konfigurasi Buku</CardTitle>
          {!canEdit && (
